...
bool createDir(const std::string& dir)
{
    ...
    return bool(CreateDirectoryA(dir.c_str(), LPSECURITY_ATTRIBUTES(NULL)));
#else
...
std::vector<std::string> listDirInDir(std::string path) {
    tinydir_dir dir;
    const std::wstring path_wide = std::wstring(path.begin(), path.end());
    const wchar_t *path_wt {path_wide.c_str()};
    tinydir_open(&dir, path_wt);
    ...
    while (dir.has_next)
    {
        ...
        if (file.is_dir)
        {
            std::wstring name_ws = file.name;
            std::string file_name_str = std::string(name_ws.begin(), name_ws.end());
            if (std::string(file_name_str) != "." && std::string(file_name_str) != "..") { fileList.push_back(std::string(file_name_str)); }
        }
        tinydir_next(&dir);
    }
    tinydir_close(&dir);
